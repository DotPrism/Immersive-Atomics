...
public class ItemBlockIABase extends ItemBlock
{
	public ItemBlockIABase(Block b)
	{
		super(b);
		if (((BlockIABase) b).enumValues.length > 1)
			setHasSubtypes(true);
	}
	...
	{
		return ((BlockIABase) this.block).getUnlocalizedName(stack);
	}
	...
	{
		if (((BlockIABase) block).hasFlavour(stack))
		{
			String subName = ((BlockIABase) this.block).getStateFromMeta(stack.getItemDamage()).getValue(((BlockIABase) this.block).property).toString().toLowerCase(Locale.US);
			String flavourKey = "desc." + ImmersivePetroleum.MODID + ".flavor." + ((BlockIABase) this.block).name + "." + subName;
	...
	{
		if (!((BlockIABase) this.block).canIEBlockBePlaced(world, pos, newState, side, hitX, hitY, hitZ, player, stack))
			return false;
		boolean ret = super.placeBlockAt(stack, player, world, pos, side, hitX, hitY, hitZ, newState);
		if (ret)
		{
			((BlockIABase) this.block).onIEBlockPlacedBy(world, pos, newState, side, hitX, hitY, hitZ, player, stack);
		}
		return ret;
	}
	...
	{
		BlockIABase blockIn = (BlockIABase) this.block;
