package com.magneticprism.immersiveatomics.client;

import com.magneticprism.immersiveatomics.common.block.helper.IABlockType;
import com.magneticprism.immersiveatomics.common.item.helper.IAFlagItem;
...
public class IAClientRenderHandler implements ItemColor, BlockColor
{
    @OnlyIn(Dist.CLIENT)
    ...
    public static IAClientRenderHandler INSTANCE = new IAClientRenderHandler();

    // Register This as the color handler for all IG Items and Blocks
    public static void register(){
        for(Supplier<Item> holder : IARegistrationHolder.getItemRegistryMap().values()){
            Item i = holder.get();
            if(i instanceof IAFlagItem){
    ...
    public int getColor(BlockState state, @Nullable BlockAndTintGetter getter, @Nullable BlockPos pos, int index) {
        if(state.getBlock() instanceof IABlockType type)
    ...
    public int getColor(ItemStack stack, int tintIndex) {
        if(stack.getItem() instanceof IAFlagItem type)
