...
import com.magneticprism.immersiveatomics.common.block.multiblock.IATemplateMultiblock;
...
public class IABlockStateProvider extends BlockStateProvider
{
    ...
    protected Logger logger = IALib.getNewLogger();

    public IABlockStateProvider(DataGenerator generator, ExistingFileHelper helper)
    {
        super(generator.getPackOutput(), IALib.MODID, helper);
    ...
    protected void registerStatesAndModels() {
        IALib.IA_LOGGER.info("Generating Multiblock Splits");
        genericmultiblock("gas_centrifuge");
    }
    ...
    private void generateGenericBlock(Block block, String block_name, String block_parent)
    {
        getVariantBuilder(block).forAllStates(state -> ConfiguredModel.builder().modelFile(models().withExistingParent(
                new ResourceLocation(IALib.MODID, "block/" + block_name).getPath(),
                new ResourceLocation(IALib.MODID, "block/" + block_parent))).build());
    }

    private void genericmultiblock(String registry_name)
    {
        IALib.IA_LOGGER.info("Generating ["+ registry_name +"] Multiblock Model Data");
        createMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), IARegistrationHolder.getMBTemplate.apply(registry_name));
    }

    private void genericmultiblockMirror(String registry_name)
    {
        IALib.IA_LOGGER.info("Generating ["+ registry_name +"] with Custom Mirror Multiblock Model Data");
        testCreateMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+"_mirrored.obj"),  (IATemplateMultiblock) IARegistrationHolder.getMBTemplate.apply(registry_name), false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, TemplateMultiblock multiblock)
    {
        createMultiblock(unsplitModel, (IATemplateMultiblock) multiblock, false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, IATemplateMultiblock multiblock, boolean dynamic)
    {
    ...
    private void testCreateMultiblock(NongeneratedModel unsplitModel, NongeneratedModel mirror_model, IATemplateMultiblock multiblock, boolean dynamic)
    {
        final ModelFile mainModel = split(unsplitModel, multiblock, false, dynamic);
    ...
    private ModelFile split(NongeneratedModel loc, IATemplateMultiblock mb)
    {
        return split(loc, mb, false);
    }

    private ModelFile split(NongeneratedModel loc, IATemplateMultiblock mb, boolean mirror)
    {
        return split(loc, mb, mirror, false);
    }

    private ModelFile split(NongeneratedModel loc, IATemplateMultiblock mb, boolean mirror, boolean dynamic)
    {
    ...
    private ModelFile split(
            NongeneratedModel name, IATemplateMultiblock multiblock, UnaryOperator<BlockPos> transform, boolean dynamic
    )
    {
    ...
    protected void createRotatedBlock(Supplier<? extends Block> block, Function<VariantBlockStateBuilder.PartialBlockstate, ModelFile> model, Property<Direction> facing,
                                      List<Property<?>> additionalProps, int offsetRotX, int offsetRotY)
    {
    ...
    public static <T extends Comparable<T>> void forEach(VariantBlockStateBuilder.PartialBlockstate base, Property<T> prop,
