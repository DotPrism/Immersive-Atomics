...
processAudio () {
    echo "Processing $1";
    bitrate=$(./gradle/tools/ffprobe -v 0 -select_streams a:0 -show_entries stream=bit_rate -of compact=p=0:nk=1 "$1");
    ...
    touch $1.tmp
    ./gradle/tools/ffmpeg -y -i $1 -f ogg -c:a libvorbis -b:a 120k $1.tmp &
    wait
    rm $1
    mv $1.tmp $1
}
